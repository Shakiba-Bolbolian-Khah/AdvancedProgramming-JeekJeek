...
string RequestHandler::makeHtmlFile(string inputText , bool isSearch ){
    string htmlFile="";
    htmlFile = readFile("htmlFiles/userHome.html");
    int numOfJeek = 0;
    if( isSearch ){
        if( inputText[0] == '#' ){
            inputText.erase( inputText.begin() );
            numOfJeek = InterFace::management->returnNumOfSearchResult( inputText );
        }
        else if( inputText[0] == '@' ){
            inputText.erase( inputText.begin() );
            numOfJeek = InterFace::management->returnNumOfJeeks( inputText );
        }
    }
    else
        numOfJeek = InterFace::management->returnNumOfJeeks( inputText );

    for( int i =0 ; i < numOfJeek ; i++ )
        htmlFile += readFile("htmlFiles/jeek.html");
    htmlFile += "</body> </html>";
    return htmlFile;
}

string  RequestHandler::makeHomeHtml(string sessionId){
    string username = InterFace::management->findUsername(sessionId);
    string homeHtml = makeHtmlFile( username , false );
